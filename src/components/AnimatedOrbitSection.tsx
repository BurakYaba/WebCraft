...
export default function AnimatedOrbitSection() {
  ...
  useEffect(() => {
    ...
    const stage = svg.querySelector(".m1_stage") as HTMLElement;

    // Set initial position - center the stage (no x transform needed, viewBox handles centering)
    ...
    tl.from(
      ...
      .add(() => {
        ...
        const handleMouseMove = (e: MouseEvent) => {
          const cGroups = svg.querySelectorAll(".m1_cGroup");
          cGroups.forEach((group, i) => {
            gsap.to(group, {
              duration: 1,
        ...
        const handleClick = () => {
          const cGroups = svg.querySelectorAll(".m1_cGroup");
          const firstGroup = cGroups[0] as HTMLElement;
          const currentScale = gsap.getProperty(firstGroup, "scale");

          if (currentScale !== 1) return; // Prevent overlapping bouncy tweens
      ...
      .fromTo(
        ...
        { xPercent: -35, yPercent: -5 },
        {
          motionPath: {
            path: () => {
              try {
                const path = MotionPathPlugin.convertToPath(".c1_line1", false);
      ...
      .fromTo(
        ...
        { xPercent: -45, yPercent: -10 },
        {
          motionPath: {
            path: () => {
              try {
                const path = MotionPathPlugin.convertToPath(".c1_line2", false);
                return path && path.length > 0 ? path[0] : "";
              } catch {
                return "";
              }
      ...
      .fromTo(
        ...
        { xPercent: -50, yPercent: -15 },
        {
          motionPath: {
            path: () => {
              try {
                const path = MotionPathPlugin.convertToPath(".c1_line3", false);
                return path && path.length > 0 ? path[0] : "";
              } catch {
                return "";
              }
      ...
      .fromTo(
        ...
        { xPercent: -50, yPercent: -25 },
        {
          motionPath: {
            path: () => {
              try {
                const path = MotionPathPlugin.convertToPath(".c1_line4", false);
                return path && path.length > 0 ? path[0] : "";
              } catch {
                return "";
              }
      ...
      .fromTo(
        ".m1OrbBlank",
        {
          x: (i: number) => 620 - (i / 4) * 380,
          transformOrigin: (i: number) => `${-(620 - (i / 4) * 380)}px 0px`,
          rotation: (i: number) => [99, -10, 55, 110, 120][i] || 0,
        },
        {
          rotation: "+=75",
          yoyo: true,
  ...
  // Responsive viewBox based on screen size
  const [viewBox, setViewBox] = useState("-800 -400 1600 800");

  useEffect(() => {
    const updateViewBox = () => {
      if (window.innerWidth < 768) {
        // Mobile: more zoomed in
        setViewBox("-600 -300 1200 600");
      } else if (window.innerWidth < 1024) {
        // Tablet: medium zoom
        setViewBox("-800 -400 1600 800");
      } else {
        // Desktop: full view
        setViewBox("-1100 -500 2200 1100");
      }
    };
  ...
  return (
    <section className="relative w-full h-[400px] sm:h-[500px] md:h-[700px] lg:h-[900px] bg-black overflow-hidden">
      <svg
        ref={svgRef}
        className="main1 absolute inset-0 w-full h-full z-10"
        viewBox={viewBox}
        ...
        <g className="m1_stage" opacity="0">
          <g className="m1_cGroup">
            ...
            <g className="m1Orb orb4b">
              <image
                href="https://assets.codepen.io/721952/logoVue.png"
                width="40"
                height="40"
              />
            </g>
          ...
          <g className="m1_cGroup">
            ...
            <g className="m1Orb orb3c">
              <image
                href="https://assets.codepen.io/721952/logoAngular.png"
                width="40"
                height="40"
              />
            </g>
            ...
            <g className="m1Orb orb3b">
              <image
                href="https://assets.codepen.io/721952/logoReact.png"
                width="40"
                height="40"
              />
            </g>
          ...
          {/* WebCraft Logo - Centered at (0, 50) - Rendered last so it's on top */}
          <g className="m1_cGroup">
            <g className="m1Orb orbCenter">
              <image
